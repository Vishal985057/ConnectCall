...
/* ─── ICE CONFIG ─────────────────────────────────────────────────
   Public TURN servers so cross-network calls (laptop ↔ mobile,
   different ISPs, behind NAT/firewalls) actually work.
──────────────────────────────────────────────────────────────── */
const ICE = [
...
/* ─── DETECT SCREEN SHARE STREAM ────────────────────────────────
   Returns true when the stream contains a displaySurface video
   track (i.e. screen share, not a camera).
──────────────────────────────────────────────────────────────── */
...
/* ─── VIDEO ELEMENT ─────────────────────────────────────────────
   ...
   FIX: re-attach srcObject every time the stream reference changes.
   FIX: screen share streams use objectFit:contain so the entire
        shared screen is visible (cover crops it).
──────────────────────────────────────────────────────────────── */
function Vid({ stream, muted, forceContain }) {
  const ref = useRef(null);
  const mutedRef = useRef(muted);

  /* Determine the correct fit: contain for screen share so nothing is cropped */
  const objectFit = forceContain || isScreenShareStream(stream) ? "contain" : "cover";
  ...
  return (
    <button
      ...
      style={{
        ...
        /* Larger touch target for mobile — at least 44×44px */
        padding:"4px 6px", minWidth:0, flexShrink:0,
      }}
    >
      <div style={{
      ...
      <span className="ctrl-lbl" style={{
        fontSize:10, color:"#9aa0a6", whiteSpace:"nowrap",
...
/* ─── TILE ──────────────────────────────────────────────────────
   FIX: For remote peers always show video if the stream has live
   video tracks. videoOn controls local track.enabled but the stream
   still exists on remotes.
   FIX: Clicking a tile in grid view now switches to speaker view
   with that participant featured (in addition to the existing pin).
──────────────────────────────────────────────────────────────── */
function Tile({ id, p, isLocal, isPinned, onPin, large }) {
  const hasVideoTrack = !!(p.stream && p.stream.getVideoTracks().some(t => t.readyState === "live"));
  const showVideo = hasVideoTrack && (isLocal ? p.videoOn !== false : true);
  const isScreen  = isScreenShareStream(p.stream);
  ...
  return (
    ...
    >
      {showVideo
        ? <Vid stream={p.stream} muted={isLocal} forceContain={isScreen} />
        : (
          <div style={{ display:"flex", flexDirection:"column",
            alignItems:"center", gap:10 }}>
            <GmAvatar name={p.name} size={large ? 80 : 52} />
          </div>
        )
      }
      {/* Bottom gradient name bar — matches Google Meet */}
      <div style={{
        ...
        {isScreen && (
          <span style={{
            fontSize:10, background:"rgba(26,115,232,.7)",
...
export default function Meet() {
  ...
  const mkPeer = useCallback(id => {
    ...
    /* Use ev.streams[0] — the authoritative stream the browser assembles.
       New MediaStream copy so React detects the reference change. */
    pc.ontrack = ev => {
      const src = ev.streams?.[0];
      if (src) {
        updPeer(id, { stream: new MediaStream(src.getTracks()) });
        src.onaddtrack = () => {
          updPeer(id, { stream: new MediaStream(src.getTracks()) });
        };
      }
  ...
  const stopShare = useCallback(async () => {
    ...
    try {
      ...
      camRef.current?.getTracks().forEach(t => t.stop());
      camRef.current = cam;
      setLocal(new MediaStream(cam.getTracks()));
      ...
      if (at) {
        Object.values(pcsRef.current).forEach(pc => {
          ...
          if (sender) sender.replaceTrack(at);
        });
      }
  ...
  const toggleShare = async () => {
    ...
    try {
      ...
      /* Replace or add video sender — if camera was off there may be no
         video sender yet. Add the track in that case. */
      Object.values(pcsRef.current).forEach(pc => {
        const sender = pc.getSenders().find(s => s.track?.kind === "video");
        if (sender) {
          sender.replaceTrack(vt);
        } else {
          pc.addTrack(vt, screen);
        }
      });
      ...
      setLocal(new MediaStream(newStream.getTracks()));
      setSharing(true);
  ...
  /* FIX: clicking a tile in grid view pins it AND switches to speaker view */
  const handleTileClick = id => {
    setPinned(p => p === id ? null : id);
    if (view === "grid") setView("speaker");
  };
            ...
            <div style={{ display:"grid",
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:18, paddingTop:4 }}>
                ...
                <button onClick={join} disabled={!myName.trim()}
                  style={{
                    padding:"15px", fontSize:16, borderRadius:4,
      ...
      <div style={{ textAlign:"center", maxWidth:360 }}>
        ...
        <button onClick={() => { sockRef.current?.disconnect(); setPhase("lobby"); }}
          style={{ background:"transparent", border:"1px solid #444746",
            borderRadius:4, padding:"12px 28px", color:"#9aa0a6",
  ...
  return (
    <div className="meet-root">
      ...
      {/* ── TOP BAR ─────────────────────────────────────── */}
      <div className="meet-topbar" style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, minWidth:0, flex:1 }}>
          ...
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{
              fontSize:13, fontWeight:500, color:"rgba(255,255,255,.85)",
              overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap",
            }}>
              ConnectCall&nbsp;·&nbsp;
              <span style={{ fontFamily:"monospace", fontSize:11, color:"rgba(255,255,255,.4)" }}>
                {code}
              </span>
            </div>
          </div>
          {isHost && (
            <span style={{
              fontSize:10, background:"rgba(26,115,232,.18)",
              color:"#8ab4f8", padding:"2px 8px", borderRadius:10, fontWeight:600,
        ...
        <div style={{
          ...
          color:"rgba(255,255,255,.5)", fontSize:12, flexShrink:0,
        }}>
          {handsN > 0 && <span style={{ color:"#fdd663" }}>✋ {handsN}</span>}
          <span style={{ display:"flex", alignItems:"center", gap:4 }}>⏱ {timer}</span>
          <span style={{
            background:"rgba(255,255,255,.08)",
            padding:"2px 7px", borderRadius:10, fontSize:12,
          }}>👥 {all.length}</span>
          <button
            onClick={() => setView(v => v==="speaker"?"grid":"speaker")}
            title={view==="speaker"?"Grid view":"Speaker view"}
            style={{
              background:"none", border:"none", cursor:"pointer",
              color:"rgba(255,255,255,.6)", display:"flex",
              alignItems:"center", padding:6, borderRadius:6,
            }}>
      ...
      <div style={{ flex:1, display:"flex", minHeight:0, overflow:"hidden" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, overflow:"hidden" }}>
          ...
          {view === "speaker" ? (
            /* ─── SPEAKER VIEW ─────────────────────────────
               Featured tile fills the space; film strip below.
               FIX: film strip uses the .filmstrip CSS class so
               thumbnails scale properly on mobile.
            ─────────────────────────────────────────────── */
            <>
              <div style={{ flex:1, padding:"8px 8px 0", minHeight:0, overflow:"hidden" }}>
              ...
              {film.length > 0 && (
                <div className="filmstrip">
                  {film.map(([id, p]) => (
                    <div key={id} className="filmstrip-tile">
                      <Tile
                        id={id} p={p}
                        isLocal={id==="local"}
                        isPinned={id===pinned}
            ...
            /* ─── GRID VIEW ────────────────────────────────
               Each tile is a 16:9 aspect-ratio wrapper.
               FIX: clicking any tile switches to speaker view
               with that participant as the featured speaker.
            ─────────────────────────────────────────────── */
            <div style={{
              ...
              {all.map(([id, p]) => (
                <div key={id} className="grid-tile-wrap">
                  <Tile
                    id={id} p={p}
                    isLocal={id==="local"}
                    isPinned={id===pinned}
        ...
        {/* ── SIDE PANEL ──────────────────────────────────
            FIX: on mobile (≤768px) this becomes a full-
            screen overlay via the .side-panel CSS class so
            it never steals video space from the call.
        ─────────────────────────────────────────────────── */}
        {panel && (
          <>
                ...
                <>
                  ...
                  <form onSubmit={sendChat} style={{
                    ...
                    <input
                      ...
                      style={{
                        flex:1, background:"#3c4043",
                        border:"1px solid rgba(255,255,255,.07)",
                        borderRadius:22, padding:"9px 14px",
                        color:"#e8eaed", fontSize:16,
                        outline:"none", fontFamily:"var(--font)",
                      }}
                    />
                    <button type="submit" disabled={!chatText.trim()} style={{
                  ...
                  {waitQ.map(u => (
                    <div key={u.id} style={{
                      ...
                      <div style={{ display:"flex", alignItems:"center",
                        ...
                        <GmAvatar name={u.name} size={36}/>
                        <span style={{ fontSize:14, fontWeight:500, color:"#e8eaed" }}>
                          {u.name}
                        </span>
                      </div>
                      <div style={{ display:"flex", gap:8 }}>
                        <button onClick={()=>admit(u.id)} style={{
                          flex:1, padding:"11px", fontSize:13,
                          borderRadius:4, background:"#1a73e8", color:"#fff",
                          border:"none", cursor:"pointer", fontFamily:"var(--font)", fontWeight:500,
                        }}>
                          Admit
                        </button>
                        <button onClick={()=>deny(u.id)} style={{
                          flex:1, padding:"11px", fontSize:13,
      ...
      {/* ── BOTTOM TOOLBAR ──────────────────────────────────
          ...
          All buttons stay in one row; nothing overflows.
      ─────────────────────────────────────────────────────── */}
      <div className="meet-toolbar">
        <div className="meet-toolbar-inner">
          {/* Mic */}
          <Ctrl
          ...
          {/* Camera */}
          <Ctrl
            icon={vidOn ? ICONS.cam : ICONS.camOff}
            label={vidOn ? "Stop video" : "Start video"}
            mode={vidOn ? "on" : "off"}
            onClick={() => setVidOn(v => !v)}
          />
          {/* Screen share — hidden on mobile as browser restricts it */}
          <Ctrl
            icon={ICONS.screen}
          ...
          <Ctrl
            ...
            onClick={toggleHand}
          />
          {/* Host waiting room button */}
          {isHost && waitQ.length > 0 && (
            <Ctrl
              icon={ICONS.lock}
              label={`Admit (${waitQ.length})`}
              mode="hi"
              badge={waitQ.length}
