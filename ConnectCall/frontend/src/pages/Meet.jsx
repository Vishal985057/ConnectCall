...
/* ─── ICE CONFIG ─────────────────────────────────────────────────
   FIX: Added public TURN servers so cross-network calls (laptop ↔
   mobile, different ISPs, behind NAT/firewalls) actually work.
   STUN-only configs fail whenever direct P2P is blocked.
──────────────────────────────────────────────────────────────── */
const ICE = [
...
/* ─── VIDEO ELEMENT ─────────────────────────────────────────────
   ...
   FIX: re-attach srcObject every time stream reference changes
──────────────────────────────────────────────────────────────── */
  ...
  return (
    <button
      ...
      style={{ display:"flex", flexDirection:"column", alignItems:"center",
        ...
        padding:"4px 2px", minWidth:0, flexShrink:0 }}
    >
      <div style={{
        width: c.w || 44, height: 44,
        borderRadius: c.r || "50%",
        background: c.bg,
      ...
      <span className="ctrl-lbl" style={{ fontSize:10, color:"#9aa0a6", whiteSpace:"nowrap",
...
function Tile({ id, p, isLocal, isPinned, onPin }) {
  /* FIX: for remote peers always show video if stream has live video tracks.
     videoOn controls local track.enabled but the stream still exists on remotes. */
...
export default function Meet() {
  ...
  const mkPeer = useCallback(id => {
    ...
    /* ─── FIX: simplified ontrack ─────────────────────────────
       Use ev.streams[0] directly — it's the authoritative stream
       the browser assembles for us. Create a new MediaStream copy
       each time so React detects the reference change and re-renders.
    ─────────────────────────────────────────────────────────── */
    pc.ontrack = ev => {
      const src = ev.streams?.[0];
      if (src) {
        updPeer(id, { stream: new MediaStream(src.getTracks()) });
        // Re-run when more tracks arrive on the same stream (e.g. audio after video)
        src.onaddtrack = () => {
          updPeer(id, { stream: new MediaStream(src.getTracks()) });
        };
      }
  ...
  const toggleShare = async () => {
    ...
    try {
      ...
      /* ─── FIX: replace or add video sender ───────────────────
         If camera was off there may be no video sender yet.
         We must add the track in that case, not just replaceTrack.
      ─────────────────────────────────────────────────────────── */
      Object.values(pcsRef.current).forEach(pc => {
        const sender = pc.getSenders().find(s => s.track?.kind === "video");
        if (sender) {
  ...
  return (
    <div style={{ height:"100dvh", background:"#1a1a1a",
      ...
      <div style={{ height:52, display:"flex", alignItems:"center",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, minWidth:0, flex:1 }}>
          ...
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ fontSize:13, fontWeight:500, color:"rgba(255,255,255,.85)",
              overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
              ConnectCall · <span style={{ fontFamily:"monospace", fontSize:11,
                color:"rgba(255,255,255,.4)" }}>{code}</span>
            </div>
          </div>
          {isHost && (
            <span style={{ fontSize:10, background:"rgba(26,115,232,.18)",
              color:"#8ab4f8", padding:"2px 8px", borderRadius:10, fontWeight:600,
              flexShrink:0, whiteSpace:"nowrap" }}>HOST</span>
          )}
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:10,
          ...
          <span style={{ background:"rgba(255,255,255,.08)",
            padding:"2px 7px", borderRadius:10, fontSize:12 }}>👥 {all.length}</span>
      ...
      {/* ─── BOTTOM TOOLBAR ──────────────────────────────────────
          FIX: Responsive — scrolls horizontally on narrow screens.
          All buttons stay in one scrollable row; nothing overflows.
      ─────────────────────────────────────────────────────────── */}
